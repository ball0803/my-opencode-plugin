...
import type { SkillsConfig, SkillDefinition } from '../../config/index.ts';
import type { BuiltinSkill } from '../builtin-skills/types.ts';
...
$ARGUMENTS
  ...
  const allowedTools =
    entry['allowed-tools'] ||
    (fileMetadata['allowed-tools']
      ? fileMetadata['allowed-tools'].split(/\s+/).filter(Boolean)
      : undefined);
