...
import type { BackgroundTaskConfig } from '../../config/index.ts';
import {
...
export class BackgroundManager {
  ...
  async launch(input: LaunchInput): Promise<BackgroundTask> {
    ...
    const createResult = await this.client.session
      .create({
        body: {
  ...
  handleEvent(event: Event): void {
    ...
    if (event.type === 'session.idle') {
      ...
      this.checkSessionTodos(sessionID).then((hasIncompleteTodos) => {
        ...
        log(
          '[background-agent] Task completed via session.idle event:',
          task.id,
        );
      });
    }
  ...
  private notifyParentSession(task: BackgroundTask): void {
    ...
    log('[background-agent] Sending notification to parent session:', {
  ...
  private async pollRunningTasks(): Promise<void> {
    ...
    for (const task of this.tasks.values()) {
      ...
      try {
        ...
        if (!sessionStatus) {
          log(
            '[background-agent] Session not found in status:',
            task.sessionID,
          );
          continue;
        }
