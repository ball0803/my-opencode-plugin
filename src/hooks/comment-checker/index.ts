...
import type { CommentCheckerConfig } from '../../config/index.ts';
...
export function createCommentCheckerHooks(config?: CommentCheckerConfig) {
  ...
  return {
    'tool.execute.before': async (
      ...
      const oldString =
        output.args.oldString ?? (output.args.old_string as string | undefined);
      const newString =
        output.args.newString ?? (output.args.new_string as string | undefined);
...
async function processWithCli(
  ...
  const hookInput: HookInput = {
    ...
    tool_name:
      pendingCall.tool.charAt(0).toUpperCase() + pendingCall.tool.slice(1),
    transcript_path: '',
    cwd: process.cwd(),
