...
import { getActiveLoop, setActiveLoop, deleteActiveLoop, clearAllLoops } from "./storage.ts"

export function createRalphLoopHook(ctx: PluginInput, options: RalphLoopOptions = {}) {
  const { config } = options

  function startLoop(sessionID: string, prompt: string, loopConfig: RalphLoopOptions["config"] = {}) {
    const state = {
      sessionID,
      prompt,
  ...
  async function checkCompletion(sessionID: string, output: string): Promise<boolean> {
  ...
  return {
    ...
    "tool.execute.after": async (input: { sessionID: string }, output: { result: string | undefined }) => {
