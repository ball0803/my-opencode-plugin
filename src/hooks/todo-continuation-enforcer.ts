...
import type { BackgroundManager } from '../features/background-agent/manager.ts';
import {
  getMainSessionID,
  subagentSessions,
...
import {
  ...
  MESSAGE_STORAGE,
} from '../features/hook-message-injector/index.ts';
...
export function createTodoContinuationEnforcer(
  ...
  async function injectContinuation(
    ...
    if (agentName === 'plan' || agentName === 'planner-sisyphus') {
      log(`[${HOOK_NAME}] Skipped: plan mode agent`, {
        sessionID,
        agent: prevMessage?.agent,
      });
      return;
  ...
  const handler = async ({
    ...
    if (event.type === 'session.idle') {
      ...
      if (mainSessionID && !isMainSession && !isBackgroundTaskSession) {
        log(`[${HOOK_NAME}] Skipped: not main or background task session`, {
          sessionID,
        });
        return;
      }
      ...
      try {
        ...
        const messages =
          (messagesResp as { data?: Array<{ info?: MessageInfo }> }).data ?? [];

        if (isLastAssistantMessageAborted(messages)) {
          log(`[${HOOK_NAME}] Skipped: last assistant message was aborted`, {
            sessionID,
          });
          return;
        }
      } catch (err) {
        log(`[${HOOK_NAME}] Messages fetch failed, continuing`, {
          sessionID,
          error: String(err),
        });
      }
