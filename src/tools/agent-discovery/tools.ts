...
const DEFAULT_AGENT_DESCRIPTIONS: Record<string, string> = {
  'explore': 'Specialized agent for exploring codebases, finding files, and analyzing code structure.',
  'librarian': 'Specialized agent for documentation tasks, reading files, and providing information.',
  'build': 'Specialized agent for build tasks, running commands, and managing project builds.',
  'test': 'Specialized agent for testing tasks, running tests, and analyzing test results.',
  'review': 'Specialized agent for code review tasks, analyzing code quality, and providing feedback.',
  ...
  return {
    ...
    async execute(options: {
      ...
      for (const agent of agents) {
        const description = includeDescriptions && DEFAULT_AGENT_DESCRIPTIONS[agent]
      ...
      output += '\n---\n';
      output += 'Use `get_agent_info` with an agent name to get more details about a specific agent.';
...
export function createGetAgentInfoTool(manager: BackgroundManager) {
  return {
    ...
    async execute(options: {
      ...
      const description = DEFAULT_AGENT_DESCRIPTIONS[agentName] || 'No description available';
