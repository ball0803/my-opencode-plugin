...
async function extractSkillBody(skill: LoadedSkill): Promise<string> {
  if (skill.lazyContent) {
    ...
    const templateMatch = fullTemplate.match(
  ...
  for (const [serverName, config] of Object.entries(skill.mcpConfig)) {
    ...
    try {
      ...
      if (resources.length > 0) {
        sections.push(
          `**Resources**: ${resources.map((r: Resource) => r.uri).join(', ')}`,
        );
      }
      if (prompts.length > 0) {
        sections.push(
          `**Prompts**: ${prompts.map((p: Prompt) => p.name).join(', ')}`,
        );
      }
...
export function createSkillTool(
  ...
  const getSkills = async (): Promise<LoadedSkill[]> => {
    ...
    cachedSkills = await discoverSkills({
      includeClaudeCodePaths: !options.opencodeOnly,
    });
    return cachedSkills;
  };
  ...
  return tool({
    ...
    args: {
      name: tool.schema
        .string()
        .describe(
          "The skill identifier from available_skills (e.g., 'code-review')",
        ),
    },
    async execute(args: SkillArgs) {
      ...
      const dir = skill.path
        ? dirname(skill.path)
        : skill.resolvedPath || process.cwd();
